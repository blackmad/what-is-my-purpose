...
(function ($) {
    ...
    const placeholder_mappings = {
        ...
        'I think your creative practice is': '... about computers talking to people',
    ...
    $("#opening > option").attr('selected',false).eq(random).attr('selected',true);
    ...
    $('.validate-form').on('submit', function(e){
        ...
        if (check) {
            ...
            $.ajax({
                type: 'POST',
                url: "https://jjt53ry4fg.execute-api.us-east-1.amazonaws.com/dev/sendCard",
                dataType: 'json',
                crossDomain: true,
                data: {
...
canvas.getContext('2d').drawImage(video,
    offset/2, 0,   // Start at 10 pixels from the left and the top of the image (crop),
    width, video.videoHeight,   // "Get" a (w * h) area from the source image (crop),
    0, 0,     // Place the result at 0, 0 in the canvas,
    width*2, video.videoHeight*2) // With as width / height: 160 * 60 (scale)
            ...
            canvas.getContext('2d').drawImage(video, 0, -offset, video.videoHeight * 2, height * 2);
        }
